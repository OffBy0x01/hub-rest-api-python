...
class BlackduckBearerAuth(requests.auth.AuthBase):
    ...
    def __init__(
        ...
        if any(arg == False for arg in (api_token, base_url)):
            raise ValueError(
                'token & base_url are required'
            )

        self.verify=verify
        self.client_token = api_token
    ...
    def authenticate(self):
        ...
        # Does not really handle exceptions - just gives more details as to possible causes
